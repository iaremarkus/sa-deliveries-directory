...
import { addAlcohol } from "models";

export const action: ActionFunction = async ({ request }) => {
  const formData = await request.formData();
  await addAlcohol(formData);
...
export default function AddAlcohol() {
  ...
  return (
    ...
    >
      ...
      <Form method="post" action="/alcohol/add" replace className="flex flex-col gap-4">
        <Fieldset disabled={transition.state === "submitting"}>
          ...
          <Datalist
            ...
            name="province"
          />
          <Input type="checkbox" label="Stockists of Wine?" name="hasWine" />
          <Input type="checkbox" label="Stockists of Spirits?" name="hasSpirits" />
          <Input type="checkbox" label="Stockists of Beer?" name="hasBeer" />
        ...
        <p className="flex flex-row gap-4 items-center justify-center">
          <button
            ...
            type="button"
            onClick={() => (confirm("Are you sure you want to cancel?") ? navigate("/alcohol") : false)}
            disabled={disabled}
          >
            Cancel
          </button>
